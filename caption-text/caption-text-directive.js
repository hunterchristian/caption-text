                ...
                controller: function ($scope, $element) {
                    ...
                    $scope.keyUpHandler = function (keyCode) {
                        if (27 === keyCode) {
                            ...
                            shadow.val($scope.placeholdertext);
                            fitTextWrap();
                        }
                    };
                    ...
                    function fitTextWrap() {
                        ...
                        if (text && text.length) {
                            while (((container.get(0).scrollHeight - 4) <= container.height() && $scope.fontsize < 1000) || ((shadow.get(0).scrollHeight - 4) <= shadow.height() && $scope.fontsize < 1000)) {
                                setFontSize(++$scope.fontsize);
                            }
                            while (((container.get(0).scrollHeight - 4) > container.height() && $scope.fontsize > 0) || ((shadow.get(0).scrollHeight - 4) > shadow.height() && $scope.fontsize > 0)) {
                                setFontSize(--$scope.fontsize);
                            }
                        }
                    }
